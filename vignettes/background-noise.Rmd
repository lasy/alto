...
alignment <- sim_data$x %>%
  run_lda_models(lda_params, reset = TRUE, dir = "./fits/background_") %>%
...
```{r}
if (params$perplexity && !is.null(params$out_dir)) {
  x_new <- simulate_gradient(N, K, V, lambdas, alpha = params$alpha)$x
  perplexities <- vector(length = params$K - 1)
  for (k in seq(2, params$n_models)) {
    load(str_c("fits/background_K", k, ".Rdata"))
    perplexities[k - 1] <- topicmodels::perplexity(tm, x_new)
  }

  data.frame(K = 2:params$n_models, perplexity = perplexities) %>%
